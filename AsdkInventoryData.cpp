...
//
static ACHAR*
strsave(const ACHAR *t)
{
    ACHAR *c = (ACHAR*)malloc((unsigned) (wcslen(t) + 1));
    if (c == NULL) {
        return c;
    }
    wcscpy(c, t);
...
void
AsdkInventoryData::setInventoryId(const ACHAR*inventoryId)
{
    if (mId != NULL) {
        free(mId);
        mId = NULL;
...
AsdkInventoryData::dxfInFields(AcDbDxfFiler* filer)
{
    ...
    if(es == Acad::eOk)
    {
        if (!filer->atSubclassData(/*NOXLATE*/L"AsdkInventoryData"))
